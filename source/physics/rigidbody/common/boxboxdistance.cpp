...
static const f32 voronoiTol = -1.0e-5f;
...
{                                                                                               \
   const f32 lsqr_tolerance = 1.0e-30f;                                                       \
   f32 lsqr;                                                                                  \
                                                                                                \
   lsqr = lsqrs.getCol##dima().get##letterb();                                                \
                                                                                                \
   if(lsqr > lsqr_tolerance) {                                                                                            \
      f32 l_recip = 1.0f/sqrtf(lsqr);                                                     \
      gap = f32(gapsAxB.getCol##dima().get##letterb())*l_recip;                           \
                                                                                                \
...
inline f32 VertexBFaceATest(bool& inVoronoi, f32& t0, f32& t1, const Vector3& hA, Vector3 faceOffsetAB, Vector3 faceOffsetBA, const Matrix3& matrixAB, const Matrix3& matrixBA, Vector3 signsB, Vector3 scalesB)
{
	// compute a corner of box B in A's coordinate system

	Vector3 corner = Vector3(faceOffsetAB + matrixAB.getCol0()*scalesB.getX() + matrixAB.getCol1()*scalesB.getY());
	...
	Vector3 facePointB = Vector3(mulPerElem(faceOffsetBA + matrixBA.getCol0()*t0 + matrixBA.getCol1()*t1 - scalesB, signsB));
...
void VertexBFaceATests(bool& done, f32& minDistSqr, Point3& localPointA, Point3& localPointB, FeatureType& featureA, FeatureType& featureB, const Vector3& hA, Vector3 faceOffsetAB, Vector3 faceOffsetBA, const Matrix3& matrixAB, const Matrix3& matrixBA, Vector3 signsB, Vector3 scalesB, bool first)
{
	f32 t0, t1;
	f32 distSqr;
...
inline f32 VertexAFaceBTest(bool& inVoronoi, f32& t0, f32& t1, const Vector3& hB, Vector3 faceOffsetAB, Vector3 faceOffsetBA, const Matrix3& matrixAB, const Matrix3& matrixBA, Vector3 signsA, Vector3 scalesA)
{
	Vector3 corner = Vector3(faceOffsetBA + matrixBA.getCol0()*scalesA.getX() + matrixBA.getCol1()*scalesA.getY());
	...
	Vector3 facePointA = Vector3(mulPerElem(faceOffsetAB + matrixAB.getCol0()*t0 + matrixAB.getCol1()*t1 - scalesA, signsA));
...
void VertexAFaceBTests(bool& done, f32& minDistSqr, Point3& localPointA, Point3& localPointB, FeatureType& featureA, FeatureType& featureB, const Vector3& hB, Vector3 faceOffsetAB, Vector3 faceOffsetBA, const Matrix3& matrixAB, const Matrix3& matrixBA, Vector3 signsA, Vector3 scalesA, bool first)
{
	f32 t0, t1;
	f32 distSqr;
...
{                                                                                               \
   ...
   edgeOffsetBA.set##bc_letter(edgeOffsetBA.get##bc_letter() - scalesB.get##bc_letter());  \
                                                                                                \
   f32 dirDot = matrixAB.getCol##bd().get##ad_letter();                                     \
   f32 denom = 1.0f - dirDot*dirDot;                                                          \
   f32 edgeOffsetAB_ad = edgeOffsetAB.get##ad_letter();                                      \
   f32 edgeOffsetBA_bd = edgeOffsetBA.get##bd_letter();                                      \
                                                                                                \
   if(denom == 0.0f)                                                                         \
      tA = 0.0f;                                                                                \
   else                                                                                         \
...
f32 EdgeEdgeTest_0101(bool& inVoronoi, f32& tA, f32& tB, const Vector3& hA, const Vector3& hB, Vector3 faceOffsetAB, Vector3 faceOffsetBA, const Matrix3& matrixAB, const Matrix3& matrixBA, Vector3 signsA, Vector3 signsB, Vector3 scalesA, Vector3 scalesB)
{
	EdgeEdgeTest(0, X, 1, Y, 0, X, 1, Y);
}

	f32 EdgeEdgeTest_0110(bool& inVoronoi, f32& tA, f32& tB, const Vector3& hA, const Vector3& hB, Vector3 faceOffsetAB, Vector3 faceOffsetBA, const Matrix3& matrixAB, const Matrix3& matrixBA, Vector3 signsA, Vector3 signsB, Vector3 scalesA, Vector3 scalesB)
{
	EdgeEdgeTest(0, X, 1, Y, 1, Y, 0, X);
}

f32 EdgeEdgeTest_1001(bool& inVoronoi, f32& tA, f32& tB, const Vector3& hA, const Vector3& hB, Vector3 faceOffsetAB, Vector3 faceOffsetBA, const Matrix3& matrixAB, const Matrix3& matrixBA, Vector3 signsA, Vector3 signsB, Vector3 scalesA, Vector3 scalesB)
{
	EdgeEdgeTest(1, Y, 0, X, 0, X, 1, Y);
}

f32 EdgeEdgeTest_1010(bool& inVoronoi, f32& tA, f32& tB, const Vector3& hA, const Vector3& hB, Vector3 faceOffsetAB, Vector3 faceOffsetBA, const Matrix3& matrixAB, const Matrix3& matrixBA, Vector3 signsA, Vector3 signsB, Vector3 scalesA, Vector3 scalesB)
{
...
void EdgeEdgeTests(bool& done, f32& minDistSqr, Point3& localPointA, Point3& localPointB, s32& otherFaceDimA, s32& otherFaceDimB, FeatureType& featureA, FeatureType& featureB, const Vector3& hA, const Vector3& hB, Vector3 faceOffsetAB, Vector3 faceOffsetBA, const Matrix3& matrixAB, const Matrix3& matrixBA, Vector3 signsA, Vector3 signsB, Vector3 scalesA, Vector3 scalesB, bool first)
{
	f32 distSqr;
	f32 tA, tB;

	s32 testOtherFaceDimA, testOtherFaceDimB;

	testOtherFaceDimA = 0;
	testOtherFaceDimB = 0;

	distSqr = EdgeEdgeTest_0101(done, tA, tB, hA, hB, faceOffsetAB, faceOffsetBA, matrixAB, matrixBA, signsA, signsB, scalesA, scalesB);
	...
	distSqr = EdgeEdgeTest_1001(done, tA, tB, hA, hB, faceOffsetAB, faceOffsetBA, matrixAB, matrixBA, signsA, signsB, scalesA, scalesB);
	...
	distSqr = EdgeEdgeTest_0110(done, tA, tB, hA, hB, faceOffsetAB, faceOffsetBA, matrixAB, matrixBA, signsA, signsB, scalesA, scalesB);
	...
	distSqr = EdgeEdgeTest_1010(done, tA, tB, hA, hB, faceOffsetAB, faceOffsetBA, matrixAB, matrixBA, signsA, signsB, scalesA, scalesB);
...
f32 boxBoxDistance(Vector3& normal, BoxPoint& boxPointA, BoxPoint& boxPointB, Box boxA, const Transform3& transformA, Box boxB, const Transform3& transformB, f32 distanceThreshold)
{
	Matrix3 identity;
	...
	f32 gap, maxGap;
	s32 faceDimA = 0, faceDimB = 0, edgeDimA = 0, edgeDimB = 0;
	...
	s32 otherFaceDimA, otherFaceDimB;
	FeatureType featureA, featureB;

	if(axisType == CROSS_AXIS) {
		EdgeEdgeTests(done, minDistSqr, localPointA_perm, localPointB_perm, otherFaceDimA, otherFaceDimB, featureA, featureB, hA_perm, hB_perm, faceOffsetAB_perm, faceOffsetBA_perm, matrixAB_perm, matrixBA_perm, signsA_perm, signsB_perm, scalesA_perm, scalesB_perm, true);
		...
		if(!done) {
			...
			if(!done) {
				EdgeEdgeTests(done, minDistSqr, localPointA_perm, localPointB_perm, otherFaceDimA, otherFaceDimB, featureA, featureB, hA_perm, hB_perm, faceOffsetAB_perm, faceOffsetBA_perm, matrixAB_perm, matrixBA_perm, signsA_perm, signsB_perm, scalesA_perm, scalesB_perm, false);
			}
		}
	} else {
