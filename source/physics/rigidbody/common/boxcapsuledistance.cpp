...
{                                                                                               \
   ...
   if(lsqr > lsqr_tolerance) {                                                                                            \
      ...
      f32 gap = f32(gapsAxB.get##lettera())*l_recip;                                      \
                                                                                                \
...
inline f32 VertexBFaceATest(bool& inVoronoi, f32& t0, f32& t1, Vector3& ptsVec, const Vector3& hA, Vector3 offsetAB, Vector3 capsDirection, f32 signB, f32 scaleB)
{
...
void VertexBFaceATests(bool& done, f32& minDistSqr, Vector3& closestPtsVec, Point3& localPointA, f32& segmentParamB, FeatureType& featureA, FeatureType& featureB, const Vector3& hA, Vector3 offsetAB, Vector3 capsDirection, f32 signB, f32 scaleB, bool first)
{
	Vector3 ptsVec;
	f32 t0, t1;
	f32 distSqr;
...
{                                                                                               \
   ...
   /* find parameters of closest points on line segments. */                                    \
                                                                                                \
   f32 capsDirection_ad = capsDirection.get##ad_letter();                                     \
   f32 ptsVec_ad = ptsVec.get##ad_letter();                                                   \
   f32 capsDirDotPtsVec = dot(capsDirection,ptsVec);                                          \
   f32 denom = 1.0f - capsDirection_ad*capsDirection_ad;                                    \
                                                                                                \
   if (denom == 0.0f)                                                                         \
      tA = 0.0f;                                                                                \
   else {                                                                                            \
...
f32 EdgeEdgeTest_01(bool& inVoronoi, f32& tA, f32& tB, Vector3& ptsVec, const Vector3& hA, f32 hB, Vector3 offsetAB, Vector3 capsDirection, Vector3 signsA, Vector3 scalesA)
{
	EdgeEdgeTest(0, X, 1, Y);
}

f32 EdgeEdgeTest_10(bool& inVoronoi, f32& tA, f32& tB, Vector3& ptsVec, const Vector3& hA, f32 hB, Vector3 offsetAB, Vector3 capsDirection, Vector3 signsA, Vector3 scalesA)
{
...
void EdgeEdgeTests(bool& done, f32& minDistSqr, Vector3& closestPtsVec, Point3& localPointA, f32& segmentParamB, s32& otherFaceDimA, FeatureType& featureA, FeatureType& featureB, const Vector3& hA, f32 hB, Vector3 offsetAB, Vector3 capsDirection, Vector3 signsA, Vector3 scalesA, bool first)
{
	Vector3 ptsVec;
	f32 tA, tB;
	s32 testOtherFaceDimA;

	testOtherFaceDimA = 0;

	f32 distSqr = EdgeEdgeTest_01(done, tA, tB, ptsVec, hA, hB, offsetAB, capsDirection, signsA, scalesA);
	...
	distSqr = EdgeEdgeTest_10(done, tA, tB, ptsVec, hA, hB, offsetAB, capsDirection, signsA, scalesA);
...
f32 boxCapsuleDistance(Vector3& normal, BoxPoint& boxPointA, CapsulePoint& capsulePointB, Box boxA, const Transform3& transformA, const Capsule& capsuleB, const Transform3& transformB, f32 distanceThreshold)
{
	...
	f32 signB = (-dot(capsDirection, axisA) > 0.0f) ? 1.0f : -1.0f;
	f32 scaleB = signB*capsuleB.hLength;
	...
	if (axisType == CROSS_AXIS) {
		EdgeEdgeTests(done, minDistSqr, closestPtsVec_perm, localPointA_perm, segmentParamB, otherFaceDimA, featureA, featureB, hA_perm, capsuleB.hLength, offsetAB_perm, capsDirection_perm, signsA_perm, scalesA_perm, true);

		if(!done)
			VertexBFaceATests(done, minDistSqr, closestPtsVec_perm, localPointA_perm, segmentParamB, featureA, featureB, hA_perm, offsetAB_perm, capsDirection_perm, signB, scaleB, false);
	} else {
		VertexBFaceATests(done, minDistSqr, closestPtsVec_perm, localPointA_perm, segmentParamB, featureA, featureB, hA_perm, offsetAB_perm, capsDirection_perm, signB, scaleB, true);

		if(!done)
			EdgeEdgeTests(done, minDistSqr, closestPtsVec_perm, localPointA_perm, segmentParamB, otherFaceDimA, featureA, featureB, hA_perm, capsuleB.hLength, offsetAB_perm, capsDirection_perm, signsA_perm, scalesA_perm, false);
	}
